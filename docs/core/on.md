...
`@dojo-ng/framework/core/on` provides event handling support with methods to attach and emit events.
...
```ts
import { emit } from '@dojo-ng/framework/core/on';
...
```ts
import { on } from '@dojo-ng/framework/core/on';
...
```ts
import { once } from '@dojo-ng/framework/core/on';
...
```ts
import { pausable } from '@dojo-ng/framework/core/on';
