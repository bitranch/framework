...
The default export of `@dojo-ng/framework/core/has` is a function which accepts a single parameter: the name of the feature to test for.
...
It's important to be able to add new feature tests that aren't provided out-of-the-box by `@dojo-ng/framework/core/has`.
...
```ts
import { add as hasAdd } from '@dojo-ng/framework/core/has';
...
`@dojo-ng/framework/core/has` maintains object hashes containing keys that correspond to all features that have been both
