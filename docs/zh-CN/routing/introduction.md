...
| **自动化代码拆分**     | 与 `@dojo-ng/cli-build-app` 结合使用，会自动按顶级路由进行代码拆分 |
...
当使用 `@dojo-ng/cli-build-app` 时，Dojo 默认支持为所有顶层路由进行自动化代码拆分。这意味着 `Route` 的 `renderer` 中引用的所有部件都会包含在该路由专有包中，当用户访问此路由时会延迟加载。
