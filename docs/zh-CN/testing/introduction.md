...
Dojo 的 `@dojo-ng/cli-test-intern` 提供了一个健壮的测试框架。它能高效地测试小部件的输出并确认是否如你所愿。
...
Dojo 使用 `@dojo-ng/cli-test-intern` 运行 `tests` 文件夹下的单元测试和功能测试。

### 运行特定的测试套件

Dojo 支持两种类型的测试方法：单元测试和功能测试。单元测试是运行在 node 和本地 [Selenium] 通道上的测试，用于测试独立的代码块。功能测试通过 Selenium 运行在浏览器中，模拟用户与软件的交互来测试整体功能。在 Selenium 上运行单元测试和功能测试时，必须先使用 `@dojo-ng/cli-build-app` 进行适当的构建。
...
包装后的测试节点，是使用 `@dojo-ng/framework/testing/renderer#wrap` 创建的，然后将其传给断言方法，作为期望输出，以代替与断言 API 交互的标准部件。注意：当在 `v()` 中使用包装的 `VNode` 时，需要使用包装节点的 `.tag` 属性，如 `v(WrappedDiv.tag, {} [])`。
